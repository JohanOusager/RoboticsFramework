...
void BoxPoseSimulation::normalKey(unsigned char key, int x, int y)
{
    ...
    case 'd':
        *_cuboids[0] *= Pose(Vector3d::ZERO(), math::toQuaternion(AxisAngle(-angle_step, Vector3d(0.0, 0.0, 1.0)))); 
        ...
        //translate
    case 't':
        *_cuboids[0] *= Pose(Vector3d(translation_step, 0.0, 0.0), Quaternion::IDENTITY()); 
        break;
    case 'g':
        *_cuboids[0] *= Pose(Vector3d(-translation_step, 0.0, 0.0), Quaternion::IDENTITY()); 
        break;
    case 'y':
        *_cuboids[0] *= Pose(Vector3d(0.0, translation_step, 0.0), Quaternion::IDENTITY()); 
        break;
    case 'h':
        *_cuboids[0] *= Pose(Vector3d(0.0, -translation_step, 0.0), Quaternion::IDENTITY()); 
        break;
    case 'u':
        *_cuboids[0] *= Pose(Vector3d(0.0, 0.0, translation_step), Quaternion::IDENTITY()); 
        break;
    case 'j':
        *_cuboids[0] *= Pose(Vector3d(0.0, 0.0, -translation_step), Quaternion::IDENTITY()); 
        break;
